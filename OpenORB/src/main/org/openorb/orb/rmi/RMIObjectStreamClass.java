...
public final class RMIObjectStreamClass
{
    ...
    // initialize some of the specialized classes
    private static RMIObjectStreamClass s_strosc = new RMIObjectStreamClass(
          java.lang.String.class, "IDL:omg.org/CORBA/WStringValue:1.0", 0L );

    private static RMIObjectStreamClass s_anyosc = new RMIObjectStreamClass(
          org.omg.CORBA.Any.class, "RMI:org.omg.CORBA.Any:0000000000000000", 0L );

    private static RMIObjectStreamClass s_tcosc = new RMIObjectStreamClass(
          org.omg.CORBA.TypeCode.class, "RMI:org.omg.CORBA.TypeCode:0000000000000000", 0L );

    private static RMIObjectStreamClass s_cdosc = new RMIObjectStreamClass( java.lang.Class.class,
          "RMI:javax.rmi.CORBA.ClassDesc:2BABDA04587ADCCC:CFBF02CF5294176B", 0x2BABDA04587ADCCCL );
    ...
    static
    {
        // constructor methods for the any/typecode
        try
        {
            Class t = RMIObjectStreamClass.class;
            s_anyosc.m_read_object_method = t.getDeclaredMethod( "readAny",
                    new Class[] { org.omg.CORBA.portable.InputStream.class } );
            s_anyosc.m_write_object_method = t.getDeclaredMethod( "writeAny",
                    new Class[] { org.omg.CORBA.portable.OutputStream.class,
                    org.omg.CORBA.Any.class } );
            s_tcosc.m_read_object_method = t.getDeclaredMethod( "readTypeCode",
                    new Class[] { org.omg.CORBA.portable.InputStream.class } );
            s_tcosc.m_write_object_method = t.getDeclaredMethod( "writeTypeCode",
                    new Class[] { org.omg.CORBA.portable.OutputStream.class,
        ...
        catch ( NoSuchMethodException ex )
        {
            throw new NoSuchMethodError(
                    "No method readAny/writeAny/readTypeCode/wrteTypeCode found (" + ex + ")" );
        }

        s_strosc.m_type = s_orb.create_value_box_tc( s_strosc.getRepoID(), "WStringValue",
                s_orb.get_primitive_tc( TCKind.tk_wstring ) );
        s_anyosc.m_type = s_orb.create_value_box_tc( s_anyosc.getRepoID(), "Any",
                s_orb.get_primitive_tc( TCKind.tk_any ) );
        s_tcosc.m_type = s_orb.create_value_box_tc( s_tcosc.getRepoID(), "TypeCode",
                s_orb.get_primitive_tc( TCKind.tk_TypeCode ) );
        ...
        s_cdosc.m_type = s_orb.create_value_tc( s_cdosc.getRepoID(), "ClassDesc",
                org.omg.CORBA.VM_NONE.value, s_orb.get_primitive_tc( TCKind.tk_null ), members );
    }

    // constructor for specialized types
    private RMIObjectStreamClass( Class clz, String repoID, long repoIDHash )
    {
        // put the new instance into the map to avoid endless loops (#787)
    ...
    private static org.omg.CORBA.TypeCode getTypeCodeFromHelper( Class helper )
        throws InvalidClassException
    {
        org.omg.CORBA.TypeCode result = null;
        try
        {
            result = ( org.omg.CORBA.TypeCode ) helper.getMethod( "type",
        ...
        catch ( InvocationTargetException ex )
        {
            ...
            throw new InvalidClassException( helper.getName(),
                    "Exception while invoking type method (" + real + ")" );
        }
        catch ( IllegalAccessException ex )
        {
            throw new InvalidClassException( helper.getName(),
                    "Exception while invoking type method (" + ex + ")" );
        }
        catch ( NoSuchMethodException ex )
        {
            throw new InvalidClassException( helper.getName(),
    ...
    // general constructor
    private RMIObjectStreamClass( Class clz, boolean suppressPackageCheck )
        throws InvalidClassException
    {
        // put the new instance into the map to avoid endless loops (#787)
        Class lookupClz = clz;
        ...
        if ( clz.isArray() )
        {
            ...
            m_type = s_orb.create_value_box_tc( getRepoID(), names[ 1 ],
                    s_orb.create_sequence_tc( 0, contained ) );
        ...
        // get serialization fields for this class or the component class
        ...
        for ( int i = 0; i < m_fields.length; ++i )
        {
            ...
            {
                // ignore this exception cause the class could override the serialPersistentFields
                // and implement it's own serialization java.math.BigDecimal !?
            }
        }

        // handle IDL entity types.
        m_idl_entity = isIDLEntity( clz );
        if ( isIDLEntity() )
        {
            if ( org.omg.CORBA.Any.class.isAssignableFrom( clz ) )
            ...
            {
                // locate the helper class for the IDLEntity
                Class helper = UtilDelegateImpl.locateHelperClass( clz );
                if ( helper == null )
                {
                    throw new InvalidClassException( "Unable to find class '" + clz.getName()
                          + "Helper'", "ClassNotFoundException" );
                }

                // get the type code from the helper class and box it
                String [] names = RepoIDHelper.mangleClassName( clz );
                m_type = s_orb.create_value_box_tc( getRepoID(), names[ 1 ],
                      getTypeCodeFromHelper( helper ) );

                // get the read/write methods
                try
                {
                    m_read_object_method = helper.getMethod( "read",
                            new Class[] { org.omg.CORBA.portable.InputStream.class } );
                    m_write_object_method = helper.getMethod( "write",
                            new Class[] { org.omg.CORBA.portable.OutputStream.class, clz } );
                }
                catch ( NoSuchMethodException ex )
                {
                    throw new InvalidClassException( helper.getName(),
                        "Couldn't find read/write method (" + ex + ")" );
                }
            }
        }
        else
        {
            if ( java.io.Externalizable.class.isAssignableFrom( clz ) )
            {
                ...
                // ensure class has a publicly accessable no-arg constructor.
                if ( !suppressPackageCheck )
                {
                    ...
                    {
                        throw new InvalidClassException( base.getName()
                              + "Missing no-arg constructor for class (" + ex + ")" );
                    }
                }
                m_externalizable = true;
                m_class_base = clz;
                // clear fields, it's not useful.
                m_fields = null;
            }
            else
            {
                ...
                if ( m_class_base != null
                      && java.io.Serializable.class.isAssignableFrom( m_class_base ) )
                {
                    try
                    {
                ...
                if ( parent != null )
                {
                    ...
                    System.arraycopy( parent.m_all_classes, 0, m_all_classes, 0,
                            parent.m_all_classes.length );
                ...
                // ensure nonserializable base class has a no-arg constructor.
                // which is accessable from this class (protected for different
                // package, package for same. Since Object is a common base
                // we explicitly don't check it.
                if ( m_class_base != null && !m_class_base.equals( java.lang.Object.class ) )
                {
                    ...
                    {
                        throw new InvalidClassException( m_class_base.getName()
                              + "Missing no-arg constructor for class (" + ex + ")" );
                    }
                    int modf = ctor.getModifiers();
                    if ( Modifier.isPrivate( modf )
                          ||  !( suppressPackageCheck
                          ||  Modifier.isPublic( modf )
                          ||  Modifier.isProtected( modf )
                          ||  m_class_base.getPackage().equals( clz.getPackage() ) ) )
                    {
                        throw new InvalidClassException( m_class_base.getName(),
                ...
                if ( !clz.isArray() )
                {
                    // get the specialized serialization methods.
            ...
            // create type codes
            short type_modifier = isCustomMarshaled() ? org.omg.CORBA.VM_CUSTOM.value
                  : org.omg.CORBA.VM_NONE.value;
            if ( baseType == null )
            {
                baseType = s_orb.get_primitive_tc( TCKind.tk_null );
            }
            org.omg.CORBA.ValueMember [] tcMembers =
                  new org.omg.CORBA.ValueMember[ m_externalizable ? totalPublic
                  : fields.length ];
            if ( fields.length > 0 )
            {
                ...
                for ( int i = 0; i < fields.length; ++i )
                {
                    if ( !m_externalizable || isPublic[ i ] )
                    {
                        ...
                        if ( sc != null )
                        {
                            if ( sc.type() == null )
                            {
                                tcMembers[ upto ].type = s_orb.create_recursive_tc(
                                        sc.getRepoID() );
                            }
                            else
                            {
                        ...
                        tcMembers[ upto ].access = isPublic[ i ]
            ...
            try
            {
                m_type = s_orb.create_value_tc( getRepoID(), "",
                        type_modifier, baseType, tcMembers );
            }
            catch ( org.omg.CORBA.BAD_PARAM ex )
            {
                throw new InvalidClassException( clz.getName(), "Duplicate member name ("
                      + ex + ")" );
            }
        }
     ...
     */
    public static RMIObjectStreamClass lookup( Class clz )
        throws InvalidClassException
    {
        ...
        if ( ret == null )
        {
            if ( !java.io.Serializable.class.isAssignableFrom( clz )
                  ...
                  || org.omg.CORBA.portable.ValueBase.class.isAssignableFrom( clz )
     ...
     * helper class of clz.
     */
    public static org.omg.CORBA.TypeCode lookupTypeCode( Class clz )
        throws InvalidClassException
    {
        if ( clz.isPrimitive() )
        {
        ...
        // Everything else is exausted. Must be an abstract interface.
    ...
    // methods used by the value handler.
     ...
     */
    boolean readObject( Object target, java.io.ObjectInputStream is )
     ...
     */
    void writeObject( Object target, java.io.ObjectOutputStream os )
        throws IOException
    {
     ...
     */
    Object readResolve( Object target )
     ...
     */
    Object writeReplace( Object target )
        throws ObjectStreamException
    {
    ...
    //
    // specialty operations to use for special objects.
    //

    private static org.omg.CORBA.Any readAny( org.omg.CORBA.portable.InputStream is )
    {
        // use the orb associated with the input stream is there is one.
    ...
    private static void writeTypeCode( org.omg.CORBA.portable.OutputStream os,
