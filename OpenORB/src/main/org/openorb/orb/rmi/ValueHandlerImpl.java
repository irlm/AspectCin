    ...
    implements javax.rmi.CORBA.ValueHandler
{
    ...
    private static DeserializationKernel s_kernel =
            DeserializationKernelFactory.retrieveDeserializationKernel();
                            ...
                            Serializable value )
    {
        ...
        //
        // we have to deal with the Java serialization mechanisms below
        //
        ...
        else
        {
            ...
            catch ( InvalidClassException ex )
            {
                ...
                {
                    getLogger().error( "Exception looking up the class "
                          + value.getClass().getName() + ".", ex );
                }
                throw new org.omg.CORBA.MARSHAL( "Exception looking up the class "
                      + value.getClass().getName() + " (" + ex + ")" );
            }
        }
        ...
        //
        // Java2Idl, 1.4.10, Custom Marshaling Format
        //
        ...
        try
        {
            ...
            {
                // a. octet - Format version. 1 (GIOP 1.2) or 2 (GIOP 1.3)
                out.write_octet( ( byte ) 1 );
                // b. (optional) Data written by writeExternal.
            ...
            for ( int i = 0; i < scc.length; ++i )
            {
                ...
                if ( scc[ i ].hasWriteObject() )
                {
                    if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                    {
                        getLogger().debug( "writeValue: hasWriteObject scc[" + i + "] = "
                              + scc[ i ] );
                    }
                    ...
                    // But when we have a writeObject() method defaultWriteObject() gets
                    // never called, so this should always be false !?
                    // There seems to be an exception: java.lang.Throwable
                    // Throwable has a writeObject() method but it just makes sure that the
                    // stackTrace member is initialized and calls defaultWriteObject()
                    // after eventually initializing the member.
                ...
                else
                {
                    if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                    {
                        getLogger().debug( "writeValue: defaultWriteObject scc[" + i + "] = "
                              + scc[ i ] );
                    }
     ...
     */
    public Serializable readValue( org.omg.CORBA.portable.InputStream in_sub,
                                   int offset,
                                   Class expected,
                                   String repoID,
                                   org.omg.SendingContext.RunTime sender )
    {
        if ( getLogger().isDebugEnabled() && Trace.isLow() )
        {
            getLogger().debug( "readValue( " + in_sub + ", " + offset
                  + ", " + expected + ", " + repoID + ", " + sender + " )" );
        }
        ...
        // handle wstrings first as they are the most commonly used type
        ...
        if ( expected != null )
        {
            // Check whether the repoID (read from the stream) matches the class that is
            // expected by the caller. If they match then there is no need to do the weird
            // 'Util.loadClass' stuff to load a class that is already know to the caller.

            // get the class name from the repo id
            ...
            if ( idxColon != -1 )
            {
                ...
                if ( clzFromRepoId.equals( expected ) )
                {
                    if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                    {
                        getLogger().debug( "readValue: clz from the repoID matches the expected"
                              + " class ( repoID=" + repoID + ", expected=" + expected + " )" );
                    }
                    clz = expected;
                }
            }
        ...
        // if the class is unknown we might need to get the codebase, i.e. URL, from which we
        // can download the class file
        if ( clz == null )
        {
            if ( getLogger().isDebugEnabled() && Trace.isMedium() )
            {
                getLogger().debug( "readValue: We try to get the class from the sending"
                      + " context now ( codebase=" + codebase + ", sender=" + sender + " )" );
            }

            // retrieve the codebase from the sending context.
            // a costly operation!
            if ( codebase == null && sender != null )
            {
                ...
                {
                    // Do an unchecked narrow for efficiency.
                    _CodeBaseStub stub = new _CodeBaseStub();
                    stub._set_delegate(
                          ( ( org.omg.CORBA.portable.ObjectImpl ) sender )._get_delegate() );
                    cb = stub;
                }
                try
                {
                ...
                {
                    // ignore. The unchecked narrow didn't work.
                }
            }

            // get the class name from the repoID
            String clzName = RepoIDHelper.unmangleRepoIDtoClassName( repoID );
            // if we can't get a class name from repoID then we need to give up
            if ( clzName == null )
            {
                if ( getLogger() != null  && getLogger().isErrorEnabled() )
                {
                    getLogger().error( "Could not convert classname from"
                         + " repository ID: \"" + repoID + "\"" );
                }
                throw new org.omg.CORBA.MARSHAL( "Could not convert classname from"
                      + " repository ID: \"" + repoID + "\"" );
            }
            ...
            try
            {
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "readValue: Util.loadClass( " + clzName + ", "
                          + codebase + ", null )" );
                }
            ...
            catch ( ClassNotFoundException ex )
            {
                ...
                throw new org.omg.CORBA.MARSHAL( "Couldn't find class "
                      + clzName + " (" + ex + ")" );
            }
        }
        ...
        catch ( InvalidClassException ex )
        {
            if ( getLogger() != null  && getLogger().isErrorEnabled() )
            {
                getLogger().error( "Class is not compatible with serialization: " + clz.toString()
                      + ".", ex );
            }
            throw new org.omg.CORBA.MARSHAL( "Class is not compatible with serialization: "
                  + clz.toString() + " (" + ex + ")" );
        }

        // we couldn't get the necessary serialization meta info for the class
        if ( sc == null )
        {
            throw new org.omg.CORBA.MARSHAL( "Unable to locate serialization data for class: "
                  + clz.toString() );
        }
        ...
        try
        {
            // create an instance of the class now
            Serializable value = null;
            try
            {
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "readValue: allocateNewObject( " + clz + ", "
                          + sc.getBaseClass() + ")" );
                }
            ...
            // put the value into the indirection table in case it is referenced
            boolean doPop = addReadIndirect( in, offset, value );
            // create an object input stream for reading the object's data
            ...
            if ( !sc.isExternalizable() )
            {
                // get the array of classes for each serializable field in the class
                RMIObjectStreamClass [] scc = sc.getAllStreamClasses();
                for ( int i = 0; i < scc.length; ++i )
                {
                    ...
                    if ( scc[ i ].hasWriteObject() )
                    {
                        if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                        {
                            getLogger().debug( "readValue: hasWriteObject scc[" + i + "] = "
                                  + scc[ i ] );
                        }

                        // "a. octet - Format version. Always 1."
                        if ( in.read_octet() != 1 )
                        {
                        ...
                        if ( in.read_boolean() )
                        {
                            // "c. (optional) Data written by defaultWriteObject.
                            //  The ordering of the fields is the same as the order in which
                            //  they appear in the mapped IDL valuetype, and these fields are
                            //  encoded exactly as they would be if the class did not have a
                            //  writeObject method."
                    ...
                    else if ( !scc[ i ].readObject( value, ois ) )
                    {
                        if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                        {
                            getLogger().debug( "readValue: defaultReadObject scc[" + i + "] = "
                                  + scc[ i ] );
                        }
                        defaultReadObject( in, value, scc[ i ] );
                    }
            ...
            // when the value has been added to the indirection stack, remove it again
            if ( doPop )
            {
                popReadIndirect();
            }
    ...
    public String getRMIRepositoryID( Class clz )
    {
        ...
        if ( org.omg.CORBA.Object.class.isAssignableFrom( clz ) )
        {
            if ( org.omg.CORBA.portable.ObjectImpl.class.isAssignableFrom( clz ) )
            {
                ...
                {
                    // return the rep id for the root class below
                }
            }
        ...
        if ( Serializable.class.isAssignableFrom( clz ) )
        {
            ...
            {
                // return a fallback rep id below
            }
        }
    ...
    public Serializable writeReplace( Serializable value )
    {
        ...
        catch ( InvalidClassException ex )
        {
            if ( getLogger() != null  && getLogger().isErrorEnabled() )
            {
                getLogger().error( "Exception while looking up the class "
                      + value.getClass().getName() + ".", ex );
            }
            throw new org.omg.CORBA.MARSHAL( "Exception while looking up the class "
                  + value.getClass().getName() + " (" + ex + ")" );
        }
        if ( descrip == null )
        {
    ...
    public Serializable writeReplaceExt( Serializable value, RMIObjectStreamClass descrip )
    {
        ...
        try
        {
            ...
            throw new org.omg.CORBA.MARSHAL(
                  "Attempted to write-replace a non-serializable value" );
        }
        catch ( java.io.ObjectStreamException ex )
        {
            ...
            throw new org.omg.CORBA.MARSHAL(
                        "Exception while writing to object stream (" + ex + ")" );
        }
    }
     ...
     */
    void defaultWriteObject( OutputStream os, Serializable value, RMIObjectStreamClass osc )
        throws NotSerializableException
    {
        if ( getLogger().isDebugEnabled() && Trace.isLow() )
        {
            getLogger().debug( "defaultWriteObject( " + os + ", "
                  + value + ", " + osc + " )" );
        }
        ...
        try
        {
            for ( int i = 0; i < fields.length; ++i )
            {
                ...
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "defaultWriteObject: writing field '"
                          + fields[ i ].getName() + "' type=" + ft.getName() );
                }
     ...
     * Create put fields.
     */
    java.io.ObjectOutputStream.PutField putFields( Serializable value, RMIObjectStreamClass osc )
     ...
     */
    void writeFields( OutputStream os, Serializable value,
        ...
        extends java.io.ObjectOutputStream.PutField
    {
        ...
        PutFieldImpl( Serializable value, RMIObjectStreamClass osc )
        {
            ...
            for ( int i = 0; i < fields.length; ++i )
            {
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "PutFieldImpl: fields[ " + i + " ] = "
                          + fields[ i ].getName() );
                }
        ...
        public void write( OutputStream os )
        {
            ...
            for ( int i = 0; i < fields.length; ++i )
            {
                ...
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "PutFieldImpl.write: fields[" + i + "].getName()=" + fldname
                          + ", value=" + val );
                }
    ...
    void defaultReadObject( InputStream is, Serializable value, RMIObjectStreamClass osc )
    {
        if ( getLogger().isDebugEnabled() && Trace.isLow() )
        {
            getLogger().debug( "defaultReadObject( " + is
                  + ", <value not yet deserialized>, " + osc + " )" );
        }
        ...
        try
        {
            for ( int i = 0; i < fields.length; ++i )
            {
                ...
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "defaultReadObject: reading field '"
                          + fields[ i ].getName() + "' type=" + ft.getName() );
                }
     ...
     * Get the read fields.
     */
    java.io.ObjectInputStream.GetField readFields( InputStream is, Serializable value,
        ...
        extends java.io.ObjectInputStream.GetField
    {
        ...
        GetFieldImpl( RMIObjectStreamClass osc, InputStream is )
        {
            ...
            try
            {
                for ( int i = 0; i < fields.length; ++i )
                {
                    ...
                    if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                    {
                        getLogger().debug( "GetFieldImpl: field[ " + i + " ] = "
                              + fields[ i ].getName() + ", type=" + ft.getName() );
                    }
                    ...
                    if ( ft.isPrimitive() )
                    {
                        if ( ft.equals( boolean.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                  ( is.read_boolean() ? Boolean.TRUE : Boolean.FALSE ) );
                        }
                        else if ( ft.equals( byte.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getByte( is.read_octet() ) );
                        }
                        else if ( ft.equals( short.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getShort( is.read_short() ) );
                        }
                        else if ( ft.equals( int.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getInteger( is.read_long() ) );
                        }
                        else if ( ft.equals( long.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getLong( is.read_longlong() ) );
                        }
                        else if ( ft.equals( float.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getFloat( is.read_float() ) );
                        }
                        else if ( ft.equals( double.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    NumberCache.getDouble( is.read_double() ) );
                        }
                        else if ( ft.equals( char.class ) )
                        {
                            m_field_values.put( fields[ i ].getName(),
                                    CharacterCache.getCharacter( is.read_wchar() ) );
                        }
                        else
                        {
                            throw new Error( "Unknown primitive type" );
        ...
        public boolean get( String str, boolean param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
        ...
        public byte get( String str, byte param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public short get( String str, short param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public int get( String str, int param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public long get( String str, long param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public float get( String str, float param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public double get( String str, double param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
        ...
        public char get( String str, char param )
            throws IllegalArgumentException
        {
            checkFieldName( str );
            Object val = m_field_values.get( str );
    ...
    private Class readClassDesc( InputStream is )
    {
        ...
        // this loading is different, it uses the class loader
        String clzName = RepoIDHelper.unmangleRepoIDtoClassName( repoID );
        if ( clzName == null )
        {
            throw new org.omg.CORBA.MARSHAL( "Could not find classname from repository ID: \""
                  + repoID + "\"" );
        }
        try
        {
    ...
    private void writeArray( OutputStream os, Serializable value )
    {
        ...
        if ( getLogger().isDebugEnabled() && Trace.isMedium() )
        {
            getLogger().debug( "writeArray: writing array of type '"
                  + cmpt.getName() + "'" );
        }
        if ( cmpt.isPrimitive() )
        {
        ...
        else
        {
            ...
            for ( int i = 0; i < len; ++i )
            {
                if ( getLogger().isDebugEnabled() && Trace.isMedium() )
                {
                    getLogger().debug( "writeArray: array[ " + i + " ] = "
                          + arr[ i ] );
                }
    ...
    private Serializable readArray( InputStream is, int offset, Class clz )
    {
        ...
        if ( getLogger().isDebugEnabled() && Trace.isMedium() )
        {
            getLogger().debug( "writeArray: reading array of type '"
                  + cmpt.getName() + "'" );
        }
        if ( cmpt.isPrimitive() )
        {
        ...
        else
        {
            ...
            if ( getLogger().isDebugEnabled() && Trace.isMedium() )
            {
                getLogger().debug( "writeArray: array of type '" + cmpt.getName()
                      + "' to be read" );
            }
    ...
    private void writeStreamValue( OutputStream os, Class decl, Object value )
    {
        ...
        else
        {
            ...
            if ( !org.omg.CORBA.Object.class.isAssignableFrom( decl )
                  && java.rmi.Remote.class.isAssignableFrom( decl )
                  && obj != null )
            {
    ...
    private boolean addReadIndirect( InputStream in, int offset, Serializable value )
    {
        if ( getLogger().isDebugEnabled() && Trace.isLow() )
        {
            getLogger().debug( "addReadIndirect( " + in + ", " + offset
                  + ", <value not yet deserialized> )" );
        }
