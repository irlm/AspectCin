...
public class StorageBuffer
{
     ...
     */
    public StorageBuffer( InputStream is, int total_len )
        throws IOException
    {
     ...
     */
    public StorageBuffer( byte [] buf, int off, int len,
                          InputStream is, int total_len )
        throws IOException
    {

        if ( buf != null
              && ( off < 0 || len < 0 || len + off > buf.length
              || ( is != null && len > total_len ) ) )
        {
            throw new IndexOutOfBoundsException();
        }
        Scrap t = new Scrap();
        ...
        {
            int alloc = ( total_len > Scrap.SCRAP_SIZE_DEFAULT )
                  ? Scrap.SCRAP_SIZE_DEFAULT : total_len;
        ...
        if ( m_read_write_mode )
        {
            while ( m_head != m_tail )
            {
                if ( m_head.m_fMode == Scrap.SCRAP_MODE_READONLY )
                {
                    ...
                    System.arraycopy( m_head.m_fBuffer, m_head.m_fOffset,
                            tmp, 0, m_head.m_fLength );
    ...
    public int next( OctetSeqHolder buf, IntHolder off, IntHolder len )
    {
        ...
        if ( len.value < m_head.m_fLength )
        {
            ...
            {
                // the real head may be used again later, use a tempory head
    ...
    public byte [] linearize()
    {
        ...
        if ( m_head.m_fLength != m_head.m_fBuffer.length || m_head.m_fNext != m_tail
              || m_head.m_fMode != Scrap.SCRAP_MODE_NORMAL || m_head.m_fOffset != 0 )
        {
            ...
            while ( nex != m_tail )
            {
                ...
                {
                    Trace.signalIllegalCondition( null,
                          "An exception occured during linearization!" );
                    // This may fail with the following exceptions:
                    // IndexOutOfBoundsException, NullPointerException, ArrayStoreException
                    // in either case copy was not successful, so return null;
                    return null;
                }
