...
public class BufferSource
{
    ...
    public StorageBuffer next()
    {
        ...
        synchronized ( this )
        {
            while ( true )
            {
                ...
                // return null if last buffer received.
                if ( m_last_received )
                {
                     break;
                }
            ...
            {      // last-listener flag set or no listener setup
                if ( m_exception != null )
                {
                    throw m_exception;      // throw the exception
                }
                return null;
            }
            // indicate last listener has been notified
            m_last_notified = true;
            // setup local version of listener object
            listenerObj = m_last_msg_processed_listener;
            // setup local version of exception object (if any)
            exceptionObj = ( m_exception != null ) ? m_exception : null;
        }

        // call listener from outside of thread-sync block to prevent
        // any chance of deadlock from listener needing 'm_sync_state'
        // lock while holding the thread lock for this object
        if ( listenerObj != null )
        {
            // listener object is setup; call its method
            listenerObj.lastMessageProcessed( this );
        }
        if ( exceptionObj != null )
        {
            // exception was setup; throw it now
            throw exceptionObj;
        }
        return null;
    }
    ...
    private void waitingForBuffer()
    {
        if ( m_waiting_for_buffer_listener != null
              && !m_waiting_for_buffer_listener.waitForBuffer( this ) )
        {
            return;
        }
        try
     ...
     */
    public synchronized void addWaitingForBufferListener( WaitingForBufferListener listener )
     ...
     * @param listener The listener to register.
     */
    public synchronized void addLastMessageProcessedListener(
            LastMessageProcessedListener listener )
        throws java.util.TooManyListenersException
    {
     ...
     * @param listener The listener to remove.
     */
    public synchronized void removeLastMessageProcessedListener(
            LastMessageProcessedListener listener )
    {
     ...
     */
    public interface WaitingForBufferListener
     ...
     * {@link #setException}.
     */
    public interface LastMessageProcessedListener
        extends java.util.EventListener
    {
